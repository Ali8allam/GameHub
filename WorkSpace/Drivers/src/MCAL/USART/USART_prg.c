...
void USART1_IRQHandler(void)
{
    ...
    /* 2. Handle Transmission Complete (TC) */
    if((GET_BIT(USART1->SR,6) == 1) && (GET_BIT(USART1->CR1, 6) == 1))
    {
        CLR_BIT(USART1->SR, 6);
        MUSART_vDisable_TC_Interrupt();
    ...
    /* 3. Handle Receive Not Empty (RXNE) - WE ADDED THIS */
    if((GET_BIT(USART1->SR, 5) == 1) && (GET_BIT(USART1->CR1, 5) == 1))
    {
        /* Jump to whatever function you passed into MUSART_vUSARTCallBack */
        if(G_Fpt[0] != NULL)
        {
            G_Fpt[0]();
        }
